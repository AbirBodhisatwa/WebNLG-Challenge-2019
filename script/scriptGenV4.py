...
def genScript(name, step, unit, layer):
    ...
    for j in range(len(PL)):
        ...
        f.write(("\n"+genOutput(VL[j], PL[j], name+idname+"-"+tag, name, step, unit, layer)).encode('utf8'))
    f.close()
    ...
    for q in range(len(SL)):
        for r in range(len(UL)):
            for s in range(len(LL)):
                idname = "_S"+SL[q]+"_U"+UL[r]+"_L"+LL[s]
                fn = NL[p]+idname+".pbs"
                if count not in doneList:
                    print("Generating #"+str(count)+": "+fn)
                    genScript(NL[p], SL[q], UL[r], LL[s])
                else:
                    print("NOT Generating #"+str(count)+": "+fn)
                count = count + 1
